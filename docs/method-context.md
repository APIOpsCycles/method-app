...
On pages without route-derived cycle or station context (including the home page and resources), the strip displays the graph-recommended cycle and continues from the recommended entry to the next relevant role stop. Route-derived cycles always take precedence when present, so manual exploration remains visible and can be classified as off-path.

Resource pages pass their generated inverse `byResource` station relationships into the resolver. When a resource supports a station on the selected perspective's recommended path, the context guidance links directly to the earliest such stop instead of offering the generic journey continuation. This keeps resource-to-work guidance graph-derived and avoids rescanning localized content in the client.
