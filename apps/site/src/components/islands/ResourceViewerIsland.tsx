...
export default function ResourceViewerIsland({ stationId, stationTitle, resources, labels }: { locale: string; labels: Record<string, string>; stationId: string; stationTitle: string; resources: ViewerResource[] }) {
  ...
  return <section className="island-panel" aria-labelledby={`resources-${stationId}`}>
    <p className="public-kicker">{labels["resources.stationKicker"]}</p><h2 id={`resources-${stationId}`}>Resources for {stationTitle}</h2>
