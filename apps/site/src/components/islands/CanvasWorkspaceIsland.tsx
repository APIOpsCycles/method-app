...
export default function CanvasWorkspaceIsland({ locale, entityId, entityTitle, canvas }: { locale: string; entityId: string; entityTitle: string; canvas: CanvasDefinition }) {
  ...
  async function importJson(file: File) { const value = parseCanvasExport(await file.text(), canvas) as { notes: Record<string, Note[]>; metadata?: Metadata }; setNotes(value.notes); if (value.metadata) setMetadata(value.metadata); setStatus("JSON imported."); }
